...
> **ステータス: Phase 8a〜8U + Phase 11〜13 — プロファイルごとに約 280 項目のワールドエディタ + バックアップ / ロールバック + ライブ言語切替 + playit.gg 公開アドレス UI。**
> プロファイル作成・初回起動で RCON が動くよう `GameUserSettings.ini` を自動生成・同梱 steamcmd でのサーバーインストール・起動 / 停止 / 再起動・RCON コマンド送信・Mod / エングラム / アイテム / 恐竜検索・Discord & Windows トースト通知・**メイン画面で英日言語をライブ切替 (再起動不要)**・**~280 項目のパラメーター編集** (世界 / 繁殖 / 戦利品 / ステータス配列 / 戦闘 / XP / チャット / クラスタ / 上限 / PvP-decay / 病気 / Cryopod / 起動フラグ / Mod リスト / MOTD) を `Game.ini` + `GameUserSettings.ini` (`[ServerSettings]` と `[MessageOfTheDay]`) + プロファイルの `MM_Command_Val` (URL + `-mods=` + `-flags`) に渡って **左サイドバー / 右ペインエディタ (18 カテゴリ + 仮想 *検索結果* ビュー)** で操作可能。各パラメーターはラベルクリックで英日説明ポップアップが出ます。
> **SavedArks の zip スナップショット** を T 時間ごとに N 個保持 (デフォルト 30 分 / 12 個、圧縮レベルは「なし / 速い / 標準 / 最大」から選択 — デフォルト Deflate 1)、各時点に **GUI からワンクリックでロールバック** (.ark / .arktribe / .arkprofile を時刻整合させて一括復元、ロールバック前自動退避付き)。
> **公開アドレス入力欄 + クリップボードコピーボタン** を Profile セクション直下に — playit.gg トンネル / Tailscale 名 / グローバル IP を一発で友人に共有可能。
...
> CLI 版 (Phase 9)、自動アップデーター、定期再起動は未実装。
...
| **バックアップ / ロールバック** | *Profile* → *Backup / Rollback* — 自動取得トグル、間隔 (T 分) / 保持数 (N 個) / 圧縮レベル設定、「今すぐ取得」、各スナップショット行の「この時点に巻き戻す」 |
...
*Profile* セクションの **Backup / Rollback** ボタンで開きます。1 ダイアログに「設定 / 手動操作 / スナップショット一覧 / ロールバック前自動退避一覧 / 確認ストリップ」を集約、コンテンツは縦スクロール可、Save / Close ボタンは底に固定。

**保存場所** — `<install>\ARKSA_Backups\<MapName>\` (ARK の `ShooterGame\Saved\` ツリーには一切干渉せず、エンジン側のフォルダ再編に巻き込まれない場所)。ファイル名は `snapshot_<MapName>_YYYYMMDD_HHMMSS.zip`、ロールバック前退避は `pre_rollback\pre_rollback_YYYYMMDD_HHMMSS.zip` (最大 3 個保持)。

**スナップショットの中身** — `SavedArks\<MapName>\` ディレクトリ全体を **1 つの zip に固める**。`.ark` (世界保存) だけでなく `<TribeID>.arktribe` (トライブ別)、`<SteamID>.arkprofile` (プレイヤー別)、`.arktribebak` / `.arkprofilebak` (時刻バックアップ)、エンジンの `.arkrbf` (rolling backup file、保持 3 はエンジン hardcode)、`LocalProfiles\` まで全部。これらは `.ark` 内が tribe ID / steam ID で互いを参照する関係なので **時刻整合した状態で一括ロールバック** しないとトライブ消失や恐竜ロストが起きます — 個別ファイル選択は意図的に未実装。

**保持ポリシー** — 「**T 時間周期で N 個保持**」のリングバッファ。デフォルト T=30 分 / N=12 (= 6 時間履歴)。範囲外の数値は読み書き両方で clamp、corrupt INI でもスケジューラが暴走 / 沈黙しないようにしてあります。

**圧縮レベル** — 4 段階から選択 (Phase 11 で導入):
...
ARK の `.ark` ファイルは半圧縮済みバイナリで「最大」にしても 30〜40% しか縮みません。一方 Deflate 9 は Deflate 1 の 5〜10 倍 CPU を食うため、デフォルトを Deflate 1 に変更しています。ディスク容量に余裕があれば「なし」にすると手動コピーと同等の瞬時バックアップに。**既存プロファイル** (`SE_BackupCompressionLevel` 未設定) は自動で Deflate 1 が適用され、コードを更新するだけで体感速度が上がります。

**自動スケジューラ** — 「定期スナップショットを取得する」を ON にすると、起動 30 秒後から 60 秒ごとに wake、現在選択中プロファイルを参照、`auto_backup_enabled` かつ「最新 snapshot から interval 分以上経過」なら snapshot + retention 整理を実行。「最終取得時刻」はディスクの newest snapshot から導出するので **ツール再起動後もスケジュールが継続** します。プロファイル切替には自動追従。
...
3. 「はい、巻き戻す」を押すと:
   1. 現在の `SavedArks\<MapName>\` を `pre_rollback\pre_rollback_<TS>.zip` に自動退避
   2. 選択したスナップショット zip を staging dir に展開 → 旧ツリーを `.replaced_<TS>` にリネーム → staging を本番へ atomic rename → 旧を削除 (途中失敗時は `.replaced_*` から復旧可能)
   3. 一覧再描画
...
| 11 | バックアップ / ロールバック (一式): `arksa-core::backup` (zip スナップショット + atomic write + staging swap rollback + zip slip 防御)、BackupWindow 一画面 (設定 / 手動 / 一覧 / pre_rollback / 確認ストリップ + 全体スクロール)、自動スケジューラ (60 秒 wake、ディスク mtime ベース、再起動継続)、4 段階圧縮レベル選択 (STORE / Deflate 1/6/9、デフォルト Deflate 1) | ✅ |
